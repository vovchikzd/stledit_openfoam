...
void toposet(const STL& object, const int& file_number) {
    auto facets = object.facets;

    auto first_extremes = facet_minmax(facets[0]);
    ...
    for (size_t i = 1; i < facets.size(); ++i) {
        auto extremes = facet_minmax(facets[i]);
