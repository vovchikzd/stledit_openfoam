...
void create_wave_stl(const std::vector<STL>& objects) {
    ...
    std::vector<std::pair<int, int>> extremes{{x_min_int, x_max_int},
                                              {y_min_int, y_max_int},
                                              {z_min_int, z_max_int}};
...
void write_wave_file(const std::vector<std::pair<int, int>>& extremes) {
    ...
    output_file << "vertex " << std::fixed
                << x_back * 1.0 << ' ' << y_right_back * 1.0 << ' '
                << z_up * 1.0 << '\n'
                << "vertex " << x_forward * 1.0 << ' '
                << y_min * 1.0 << ' ' << z_up * 1.0 << '\n'
                << "vertex " << x_forward * 1.0 << ' '
                << y_max * 1.0 << ' ' << z_up * 1.0 << '\n'
                << "endloop\nendfacet\n";
    ...
    output_file << "facet normal " << std::fixed
                ...
                << ' ' << (x_forward - x_back) * (z_bottom - z_up) * 1.0
                << ' ' << 0.0 << '\n';
    output_file << "outer loop\n";
    output_file << "vertex " << std::fixed
                << x_back * 1.0 << ' ' << y_right_back * 1.0 << ' '
